...
import { ImageDescriptionGenerator } from "../../Scripts/ImageDescriptionGenerator";
...
export class CatalogueItem3D extends BaseScriptComponent {
    ...
    @input
    private enableImageDescription: boolean = true;
    ...
    private uniqueMaterial: Material | null = null;
    ...
    onAwake() {
        ...
        // Create unique material for the image to prevent sharing issues
    ...
    /**
     * Create a unique material instance for this catalogue item to prevent texture sharing
     */
    private createUniqueMaterial(): void {
        if (this.itemImage) {
            // Get the current material and create a copy
            const currentMaterial = this.itemImage.mainMaterial;
            if (currentMaterial) {
                // Create a unique material by copying the existing one
                this.uniqueMaterial = currentMaterial.clone();
                this.itemImage.mainMaterial = this.uniqueMaterial;
                print(`Created unique material for item: ${this.itemData?.name || 'Uninitialized'}`);
            }
        }
    }
    ...
    public setItemTexture(texture: Texture): void {
        if (this.itemImage && texture) {
            ...
            // Use the unique material to prevent sharing issues
            if (this.uniqueMaterial) {
                this.uniqueMaterial.mainPass.baseTex = texture;
            } else {
                // Fallback: set directly on the image's material
    ...
    public setPlaceholderImage(): void {
        if (this.itemImage) {
            ...
            // Set to white color as placeholder using unique material
            if (this.uniqueMaterial) {
                this.uniqueMaterial.mainPass.baseColor = new vec4(1, 1, 1, 1);
                this.uniqueMaterial.mainPass.baseTex = null; // Clear any texture
            } else {
                // Fallback: set directly on the image's material
                this.itemImage.mainPass.baseColor = new vec4(1, 1, 1, 1);
                this.itemImage.mainPass.baseTex = null;
            }
    ...
    private onItemPressed(): void {
        ...
        if (this.hasTexture()) {
            ...
            // Trigger image description if enabled and generator is available
            if (this.enableImageDescription && this.imageDescriptionGenerator && this.currentTexture) {
                print("🔍 Starting AI image description...");
    ...
    /**
     * Trigger AI description of the current item's image
     */
    private describeCurrentImage(): void {
        if (!this.imageDescriptionGenerator) {
            print("CatalogueItem3D: No ImageDescriptionGenerator available");
            return;
        }

        if (!this.currentTexture) {
            print("CatalogueItem3D: No texture available for description");
            return;
        }

        print(`🤖 Describing image for item: ${this.itemData?.name || 'Unknown'}`);

        this.imageDescriptionGenerator.describeImage(this.currentTexture)
            .then((description) => {
                print(`🎯 AI Description for "${this.itemData?.name}": ${description}`);
                this.onImageDescriptionReceived(description);
            })
            .catch((error) => {
                print(`❌ Image description failed for "${this.itemData?.name}": ${error}`);
                this.onImageDescriptionError(error);
            });
    }

    /**
     * Called when AI description is successfully received
     * Override this method for custom behavior
     */
    protected onImageDescriptionReceived(description: string): void {
        // Default behavior: just log the description
        print(`✅ "${this.itemData?.name}" Description: ${description}`);
    ...
    /**
     * Called when AI description fails
     * Override this method for custom error handling
     */
    protected onImageDescriptionError(error: string): void {
        // Default behavior: just log the error
        print(`❌ Description Error for "${this.itemData?.name}": ${error}`);
    ...
    /**
     * Manually trigger image description (useful for external calls)
     */
    public triggerImageDescription(): void {
        if (this.hasTexture()) {
            this.describeCurrentImage();
        } else {
            print("CatalogueItem3D: Cannot describe image - no texture available");
        }
    }
